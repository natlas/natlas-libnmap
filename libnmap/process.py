    ...
    def __init__(
        ...
        if self.__nmap_binary is None:
            raise EnvironmentError(
                1, "nmap is not installed or could " "not be found in system path"
            )
        ...
        if safe_mode and not self._nmap_options.isdisjoint(unsafe_opts):
            raise Exception("unsafe options activated while safe_mode " "is set True")
        ...
        try:
            ...
            for xlmnt, xmlnode in edomdoc:
                if xlmnt is not None and xlmnt == pulldom.START_ELEMENT:
                    if xmlnode.nodeName == "taskbegin" and xmlnode.attributes.keys():
                        ...
                        xmlnode.nodeName == "taskprogress" and xmlnode.attributes.keys()
                    ):
                        xt = xmlnode.attributes
                        tname = xt["task"].value
...
def main():
    ...
    nm = NmapProcess("scanme.nmap.org", options="-A", event_callback=mycallback)
    rc = nm.run()
    if rc == 0:
        print("Scan started at {0} nmap version: {1}").format(nm.starttime, nm.version)
